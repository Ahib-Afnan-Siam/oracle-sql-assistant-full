// src/components/TestTokenUsageAPI.tsx
import React, { useState, useEffect } from 'react';

const TestTokenUsageAPI: React.FC = () => {
  ...
  return (
    <div className="p-6 bg-white rounded-lg shadow dark:bg-gray-800">
      <h2 className="text-xl font-bold mb-4 dark:text-white">Token Usage API Test</h2>
      {loading && <p className="dark:text-gray-300">Loading token usage data from database...</p>}
      {error && <p className="text-red-500 dark:text-red-400">Error: {error}</p>}
      {tokenUsageData && (
        <div className="mt-4">
          <h3 className="text-lg font-medium dark:text-gray-100">Token Usage Statistics</h3>
          ...
          <h3 className="text-lg font-medium dark:text-gray-100 mt-6">Token Usage by Model</h3>
          ...
          <div className="mt-6 p-4 bg-green-50 dark:bg-green-900/20 rounded-lg">
            <p className="text-green-800 dark:text-green-200">
              ✅ Verification successful: The API endpoint is correctly returning token usage data from the database.
            </p>
          </div>
        </div>
      )}
...
export default TestTokenUsageAPI;