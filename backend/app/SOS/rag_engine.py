...
from .query_engine import _MONTH_ALIASES
from .vector_store_chroma import hybrid_schema_value_search
...
from .query_engine import _get_table_colmeta
from functools import lru_cache
from .summarizer import summarize_results_async
...
from .summarizer import summarize_with_mistral, _fallback_summarization

# Add this import to ensure we're using the API-based summarizer
from .summarizer import summarize_results


from .query_classifier import has_visualization_intent
...
async def _try_hybrid_processing(
    ...
    try:
        ...
        try:
            ...
            # Note: We don't have a record_processing_error function, so we'll skip this for now
            logger.warning("[RAG] Skipping processing error recording - function not available")
    
    # This return None was causing the syntax error - it should be inside the try block above
    # return None
    except Exception as e:
        logger.error(f"[RAG] Unexpected error in hybrid processing: {e}")
        return None

    # Move the code that was after the return statement to the correct place
    rows = []
...
"""
        ...
        try:
            ...
            model = API_MODELS["general"]["primary"]  # Use configured model instead of hardcoded one
            # Create the request payload
            from app.config import SUMMARIZATION_CONFIG
